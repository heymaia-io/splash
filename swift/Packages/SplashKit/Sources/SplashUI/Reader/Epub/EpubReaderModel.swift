...
public final class EpubReaderModel: Identifiable {
    ...
    public init(
        book: SplashBook, source: EpubSource, api: any KomgaApi, settings: EpubReaderSettingsRepository,
    ...
    static var device: R2Device {
        #if canImport(UIKit)
        R2Device(id: UIDevice.current.identifierForVendor?.uuidString ?? "splash-ios", name: "Splash for iOS")
        #else
        R2Device(id: "splash-macos", name: "Splash")
        #endif
    }
}

/// Implemented outside this package (Readium lives in `SplashEpubKit`, iOS-only) and injected by the app.
@MainActor
