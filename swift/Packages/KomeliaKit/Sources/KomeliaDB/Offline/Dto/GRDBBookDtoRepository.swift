...
struct GRDBBookDtoRepository: OfflineBookDtoRepository {
    ...
    private func sibling(bookId: KomgaBookId, userId: KomgaUserId, next: Bool) throws -> KomeliaBook? {
        guard
    ...
    private func fetch(userId: KomgaUserId, where whereClause: SQL, suffix: SQL) throws -> [KomeliaBook] {
        let counts = try [
    ...
    private func book(from row: Row, children: BookMetadataChildren) throws -> KomeliaBook {
