...
public struct RemoteBookApi: KomgaBookApi {
    ...
    public func getOne(_ bookId: KomgaBookId) async throws -> KomeliaBook {
        let book: KomgaBook = try await http.fetch(http.request(.get, "api/v1/books/\(bookId)"))
        return try await komeliaBook(book)
    }

    public func getBookList(search: KomgaBookSearch, pageRequest: KomgaPageRequest?) async throws -> Page<KomeliaBook> {
    ...
    public func getLatestBooks(pageRequest: KomgaPageRequest?) async throws -> Page<KomeliaBook> {
        var query: [URLQueryItem] = []
        query.appendPage(pageRequest)
    ...
    public func getDuplicateBooks(pageRequest: KomgaPageRequest?) async throws -> Page<KomeliaBook> {
        var query: [URLQueryItem] = []
        query.appendPage(pageRequest)
    ...
    public func getBookSiblingPrevious(_ bookId: KomgaBookId) async throws -> KomeliaBook? {
        try await sibling("api/v1/books/\(bookId)/previous")
    }

    public func getBookSiblingNext(_ bookId: KomgaBookId) async throws -> KomeliaBook? {
    ...
    // MARK: - KomeliaBook mapping (getKomeliaBook / getKomeliaBookPage)

    private func sibling(_ path: String) async throws -> KomeliaBook? {
        guard let book: KomgaBook = try await http.nilIfNotFound({ try await http.fetch(http.request(.get, path)) })
        else { return nil }
        return try await komeliaBook(book)
    }

    private func komeliaBook(_ book: KomgaBook) async throws -> KomeliaBook {
        let state = try await offlineBooks?.offlineStates(for: [book.id])[book.id]
        return KomeliaBook(book: book, offlineState: state)
    }

    private func komeliaPage(_ page: Page<KomgaBook>) async throws -> Page<KomeliaBook> {
        let states = try await offlineBooks?.offlineStates(for: page.content.map(\.id)) ?? [:]
        return page.map { KomeliaBook(book: $0, offlineState: states[$0.id]) }
    }
}
